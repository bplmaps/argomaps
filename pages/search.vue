...
export default {
	...
	beforeMount() {
		...
		// }
		// is there a JS way to pull query and search?
		let params = (new URL(document.location)).searchParams;
		if (params.get("q")) {
			let search_query = params.get("q");
			if (search_query.trim()) {
				this.search_items.push({'query':search_query, 'show_query':search_query, 'type':'keyword', 'search_field':'all_fields', 'live_search':true, 'local_filter':false});
			}
		}
	...
	methods: {
		...
		search: function(loadMore=false) {
			console.log('> search', loadMore, this.search_items);
			this.isLoading = true;
