  ...
  <div class="map-result thumbnail-view">
    ...
    <div v-else>
      <div class="map-details-container">
        <h1 class="text-3xl">
          {{ metadataAPIvariables.response.document.title_info_primary_tsi }}
          <span
            class="subtitle"
            v-if="
              metadataAPIvariables.response.document
        ...
        <p
          class="intro"
          v-html="metadataAPIvariables.response.document.abstract_tsi"
        ></p>
      </div>
      ...
      <div class="map-details-container map-details">
        ...
        <div class="meta-info">
          ...
          >
            <a
              :href="
                JSON.parse(
                  metadataAPIvariables.response.document
                    .related_item_isreferencedby_ssm[0]
                ).url
              "
              target="_blank"
              class="button-like primary no-arrow"
              >See this object’s record at
              {{
                metadataAPIvariables.response.document.institution_name_ssi
              }}</a
            >
          </div>
          ...
          <ul class="tagged-with">
            <li v-for="(tag, tag_key) in this_map.map_tags" :key="tag_key">
              <a
                :href="
                  '/explore/' + tag.order + tag.category + '-' + tag.group_order
                "
                >{{ tag.name }}</a
              >
            </li>
          ...
          <h2
            v-if="metadataAPIvariables.response.document.exemplary_image_ssi"
            class=""
          >
            Downloads
          </h2>
          <a
            ...
            :href="
              ...
              metadataAPIvariables.response.document.exemplary_image_ssi +
              '/full/full/0/default.jpg'
            "
            class="button-like"
            :download="this_map.exemplary_image_ssi + '_thumbnail_300.jpg'"
            target="_blank"
            >Large Image</a
          >
          <a
            v-if="metadataAPIvariables.response.document.exemplary_image_ssi"
            :href="
              'https://bpldcassets.blob.core.windows.net/derivatives/images/' +
              metadataAPIvariables.response.document.exemplary_image_ssi +
              '/image_thumbnail_300.jpg'
            "
            class="button-like"
            :download="this_map.exemplary_image_ssi + '_thumbnail_300.jpg'"
            target="_blank"
          ...
          <div>
            <a
              :href="`https://collections.leventhalmap.org/search/${$nuxt.context.params.solr_id}/manifest.json`"
              target="_blank"
              class="button-like dark"
              >IIIF Manifest</a
            >
            <a
              :href="`https://collections.leventhalmap.org/search/${$nuxt.context.params.solr_id}?format=json`"
              target="_blank"
              class="button-like dark"
              >JSON Metadata</a
            >
...
export default {
  ...
  async fetch() {
    this.metadataAPIvariables = await fetch(
      `https://collections.leventhalmap.org/search/${this.$nuxt.context.params.solr_id}.json`
    ).then((d) => d.json());
  },

  fetchOnServer: function () {
    return this.$nuxt.context.app.head.title === "ARGO - STAGING"
      ? false
      : true;
  },
  ...
  mounted() {
    ...
    let t = new Tify({
      ...
      manifestUrl:
        ...
        this.$nuxt.context.params.solr_id +
        "/manifest.json",
      viewer: {
    ...
    t.ready.then(() => {
		...
		// can't figure out why t.viewer is undefined at ready time, so using this very inelegant workaround
		// best guess: probably because viewer isn't instantiated until the Manifest fetch loads? 
      setTimeout(() => {
        t.viewer.addHandler("canvas-scroll", function (event) {
