	...
	<div><!-- version 2 -->
		...
		<div class="map-background">
			<div class="campaign">
				...
				<p>Discover the historical geography of the American Revolutionary War era through maps, rich interpretive essays, and interactives</p>
				<div><form action="/search/" @submit.prevent="searchSubmitFromHomepage">
					<label for="header-search" class="sr-only">Search</label>
					<input placeholder="Search" v-model="search_on_homepage" id="header-search" type="text"><button><em class="fa fa-search"></em></button>
				</form></div>
			</div>
		...
		<div id="app" class="home-content">
			<div class="explore-by curated-stories-slider">
					...
					<div class="swiper-wrapper">
						<div class="swiper-slide">
							<a href="/story-forgotten-theater/">
								<div class="swiper-image" style="background-image: url('/images/stories_forgotten_theater.jpg');"></div>
								<p>The Forgotten Theater of the American Revolution</p>
							</a>
						</div>
						<div class="swiper-slide">
							<a href="/explore/exhibits-0/">
								<div class="swiper-image" style="background-image: url('/content/campaigns/facets_29_image_compress_100.jpg');"></div>
								<p>Mapping the American Revolution</p>
							</a>
						</div>
						<div class="swiper-slide">
							<a href="/explore/exhibits-1/">
								<div class="swiper-image" style="background-image: url('/content/campaigns/facets_28_image_compress_100.jpg');"></div>
				...
				<div class="curated-stories-swiper-button-next swiper-button-next"></div>
				<div class="curated-stories-swiper-button-prev swiper-button-prev"></div>
			</div>
			...
			<div :class="['explore-by explore-by-outer explore-by-timelines',{'explore-by-timelines-active': show_pane}]">
				...
				<div class="timeline-years" @scroll.passive="timelineScroll()" v-if="ready && !is_mobile"><!-- if it's NOT mobile -->
					<div class="years">
						<div class="year" :id="'year-'+year" :key="year" v-for="index, year in maps_by_years">
							<div class="year-orient" :id="'year-orient-'+year" >
								<div class="era" v-if="timeline_eras[year]">
									<div class="era-headline">{{timeline_eras[year].era}} &nbsp; <span class="era-years">{{timeline_eras[year].era_start_year}} - {{timeline_eras[year].era_end_year}}</span></div>
									<img class="era-image" :src="timeline_eras[year].background_image" />
							...
							<div class="map-count">
								<template v-if="maps_by_years[parseInt(year)].length">
									<a @click="drawer_results_visible = maps_by_years[parseInt(year)]" class="tooltip">
										<div class="top">
									...
									<summary class="event-name">
										<div class="event-right">
											<div v-if="event.solr_ids_array" class="map-count">
												<a @click="drawer_results_visible = event.solr_ids_array" class="tooltip">
													<div class="top">{{event.solr_ids_array.length}} Related Maps<i></i></div>{{event.solr_ids_array.length}}
												</a>
											</div>
										</div>
										{{event.name}} {{event.end_year}}
									</summary>
									<div class="event-notes">{{event.event_date_range}}<br /><br />{{event.notes}}</div>
								</details>
							</div>
						</div>
					</div>
				</div>
				<div class="timeline-years" @scroll.passive="timelineScroll()" v-if="ready && is_mobile"><!-- if it IS mobile -->
					<div class="years" v-for="index, year in maps_by_years" :key="year">
						...
						<div class="year" :id="'year-'+year">
							<div class="year-text" :class="{ 'no-border' : timeline_eras[parseInt(year) + 1]}">{{year}}</div>
								...
								<details class="event" v-for="event, index in timeline_events[year]" :key="index">
									<summary class="event-name">
										<div class="event-right">
											<div v-if="event.solr_ids_array" class="map-count">
												<a @click="drawer_results_visible = event.solr_ids_array" class="tooltip">
													<div class="top">{{event.solr_ids_array.length}} Related Maps<i></i></div>
				...
				<div class="explore-by-header">
					...
					<ul class="explore-by-tags">
						<template v-for="tag_group, index in tag_groups">
							<li :key="index" v-if="index != 'institution_name_ssi' && index != 'years'"><a @click="clickFacet(index)" :class="[{active: show_facet==index}]">{{tag_group.show_name}}</a></li>
						</template>
					</ul>
				</div>
				<template v-for="tag_group, index in tag_groups" >
					<div :key="index" v-if="show_facet==index && index != 'institution_name_ssi' && index != 'exhibits' && index != 'years'"><!-- NOT mobile -->
			...
			<div class="explore-by explore-by-institution" v-if="ready == true"><!-- NOT mobile -->
				<div class="explore-by-header">
					<h2>Explore by Institutions</h2>
				</div>
				<div class="swiper institutions-swiper">
					<div class="swiper-wrapper align-bottom">
						<a @click="drawer_results_visible = tag.solr_ids_array" v-for="tag, tag_index in tag_groups.institution_name_ssi.data" :key="tag_index" class="swiper-slide explore-slide">
				...
				<div class="map-container">
					<div>
						...
						<div class="explore-by-checkboxes">
							<label :class="['explore-map-check','xl-map',{'label-checked': true}]"><input name="map-sizes[]" v-model="map_form.sizes" value='XL' @change="changeLayers()" type="checkbox"><span class="checkmark"></span>Very large (country)<svg width="20px" height="20px"><use href="#map-dot"></use></svg></label>
							<label :class="['explore-map-check','l-map',{'label-checked': true}]"><input name="map-sizes[]" v-model="map_form.sizes" value='L' @change="changeLayers()" type="checkbox"><span class="checkmark"></span>Large (region)<svg width="20px" height="20px"><use href="#map-dot"></use></svg></label>
							<label :class="['explore-map-check','m-map',{'label-checked': true}]"><input name="map-sizes[]" v-model="map_form.sizes" value='M' @change="changeLayers()" type="checkbox"><span class="checkmark"></span>Medium (colony)<svg width="20px" height="20px"><use href="#map-dot"></use></svg></label>
							<label :class="['explore-map-check','s-map',{'label-checked': true}]"><input name="map-sizes[]" v-model="map_form.sizes" value='S' @change="changeLayers()" type="checkbox"><span class="checkmark"></span>Small (city)<svg width="20px" height="20px"><use href="#map-dot"></use></svg></label>
							<label :class="['explore-map-check',{'label-checked': true}]"><input name="map-sizes[]" v-model="map_form.sizes" value='A' @change="changeLayers()" type="checkbox"><span class="checkmark"></span>Include maps without sizes<svg width="20px" height="20px"><use href="#map-dot"></use></svg></label>
						</div>
					</div>
					<div id="map-holder" :class="[{'selecting': map_selecting_location}, {'geolocating': map_geolocating_location}]">
						<div id="map-column-holder">
							<div id="map-tools">
								<form method="post" @submit.prevent="fetchLocationDataByAddress">
									<div id="map-tools-address-holder">
										<label id="map-tools-address-label" for="map-tools-address-input">Input city, region or state to drop a pin to find maps nearby.<i></i></label>
										...
										<div id="map-tools-address-pulldown" v-if="map_form_input_pulldown_toggle">
											<div @click="chooseMapPlace(pulldown_index)" v-for="pulldown_arr, pulldown_index in map_form_input_pulldown_arr" :key="pulldown_index">{{pulldown_arr.display_name}}</div>
										</div>
									</div>
								...
								<a class="tooltip" @click="chooseLocationFromBrowser()" id="map-tools-choose-from-browser">
									...
									<div class="top">Request current location from browser<i></i></div>
								</a>
								<a class="tooltip" @click="chooseLocationInMap()" id="map-tools-choose-in-map">
									<img src="/images/icon-map_pin.svg" alt="Drop a pin">
									<div class="top">Enable to click on map to choose a location<i></i></div>
								</a>
							</div>
							...
							<div id="map-key" class="hidden">
								...
								<button class="button-like primary" @click="drawer_results_visible = map_filter_results;">Show Maps ({{map_filter_results.length}})</button>
	...
	methods: {
		...
		fetchLocationDataByAddress: function() {
			...
			if (form_input.trim().length > 3) {
				...
				fetch('https://nominatim.openstreetmap.org/search.php?q='+encodeURIComponent(form_input.trim())+'&format=jsonv2&&viewbox=-107.5,7.0,-42.5,69.5&polygon_geojson=1')
					...
					.then((response) => {
						...
						for (var i=0;i<response.length;i++) {
							this.map_form_input_pulldown_arr.push({'display_name': response[i].display_name, 'coordinates': response[i].geojson.coordinates[0], 'lat':response[i].lat, 'lon':response[i].lon, 'lon1':response[i].boundingbox[0], 'lon2':response[i].boundingbox[1], 'lat1':response[i].boundingbox[2], 'lat2':response[i].boundingbox[3]})
						}
					}
				)
				.catch(function(err) {
		...
		chooseMapPlace: function (pulldown_index) {
			console.log('> chooseMapPlace', this.map_form_input_pulldown_arr[pulldown_index]);
			...
			if (typeof this.map_form_input_pulldown_arr[pulldown_index].coordinates == 'object') {
				let mapshape = new ol.Feature({
					geometry: new ol.geom.Polygon([this.map_form_input_pulldown_arr[pulldown_index].coordinates]).transform('EPSG:4326','EPSG:3857'),
				})
				mapshape.map_title = 'Map Location';
				mapshape.setStyle(new ol.style.Style({
			...
			// zoom into extent
			this.map.getView().fit(ol.proj.transformExtent([this_map.lat1, this_map.lon1, this_map.lat2, this_map.lon2], 'EPSG:4326', this.map.getView().getProjection()), { size: this.map.getSize(), duration: 1000 });
		...
		setupMap: function() {
			...
			Object.keys(this.maps).forEach(this_map => {
				if (this.maps[this_map].subject_bbox_geospatial) {
					// add a bbox
					let map_coordinates_arr = this.maps[this_map].subject_bbox_geospatial.split(', ');
					let map_coordinates_polygon = [[map_coordinates_arr[0], map_coordinates_arr[2]],[map_coordinates_arr[0], map_coordinates_arr[3]],[map_coordinates_arr[1], map_coordinates_arr[3]],[map_coordinates_arr[1], map_coordinates_arr[2]]];
					let map_point_arr = [(parseFloat(map_coordinates_arr[2]) + parseFloat(map_coordinates_arr[3])) / 2, (parseFloat(map_coordinates_arr[0]) + parseFloat(map_coordinates_arr[1])) / 2];
					// console.log('point--->', map_coordinates_arr, map_point_arr);

					// figure out the area / size
					let area = Math.abs( (map_coordinates_arr[1] - map_coordinates_arr[0]) * (map_coordinates_arr[3] - map_coordinates_arr[2]));
					let area_scale = .5;
					if (area >= 1 && area < 10) area_scale = .7;
					if (area >= 10 && area < 100) area_scale = .85;
					...
					mapshape.map_institution_name = this.maps[this_map].institution_name_ssi;
					...
					let mappoint = new ol.Feature({
						geometry: new ol.geom.Point(ol.proj.fromLonLat([map_point_arr[1], map_point_arr[0]]))
					})
					...
					mappoint.map_institution_name = this.maps[this_map].institution_name_ssi;
					...
					let map_point_arr = this.maps[this_map].subject_point_geospatial.split(',');
			...
			const end = performance.now(); let exeTimes = (end - start); console.log('map setup', exeTimes);
			...
			this.map.on('click', (evt) => {
				...
				if (this.map_selecting_location) {
					...
					if(this.map_pin || this.map_circle || this.map_search_shape) this.clearLocation();

					// currently selecting a location, so drop a pin here
					let clicked_coordinate = ol.proj.transform(evt.coordinate, 'EPSG:3857', 'EPSG:4326');
				...
				this.map.forEachFeatureAtPixel(evt.pixel, (feature) => {
					if(feature.name != "current_requested_circle" ){ //ignore the circle for clicky
				...
				if (feature) {
					...
					for(let i=0; i < assocLayers.length; i++){
						if( assocLayers[i].get('map_solr_id') == feature.map_solr_id && assocLayers[i].get('map_type') == 'area_map_shape'){
							targetLayer = assocLayers[i]; 
							break;
						}
					} 
					if (targetLayer) {
						console.log('show the map rectangle', targetLayer, feature.map_solr_id);
					...
					if (!clickedItemPosition) {
						console.log('clicked on some other non-map it seems', feature.get("name"), clickedItemPosition, feature.getGeometry(), feature.getGeometry().getCoordinates());
						return;
					} 
					...
					if (typeof clickedItemPosition[0]=='number') {
						...
						avgx = feature.getGeometry().getExtent()[2] - (feature.getGeometry().getExtent()[2] - feature.getGeometry().getExtent()[0]) / 2;
						avgy = feature.getGeometry().getExtent()[3] - (feature.getGeometry().getExtent()[3] - feature.getGeometry().getExtent()[1]) / 2;
					...
					<div><p><b>${feature.map_title}</b></p>
						...
						if (feature.map_type=='area_map_point') newhtml += `<button type="button" id="popup_zoom" class="button-like" >Zoom to Map</button>&nbsp;`;
					...
					this.$nextTick( () => {
						...
						if (popup_zoom_ele) {
							popup_zoom_ele.onclick = () => {
								...
								let map_coordinates_arr = this.maps[feature.map_solr_id].subject_bbox_geospatial.split(', ');
								let extent = ol.extent.applyTransform([map_coordinates_arr[1],map_coordinates_arr[3],map_coordinates_arr[0],map_coordinates_arr[2]], ol.proj.getTransform("EPSG:4326", "EPSG:3857"));
								// zoom in, with animation and padding
								this.map.getView().fit(extent, {duration: 1000, size: this.map.getSize(), padding: [50, 50, 50, 300]});
					...
					console.log('no feature detected at: ',ol.proj.transform(evt.coordinate, 'EPSG:3857', 'EPSG:4326'));
				}
			});
			...
			for(let i=0; i < allLayers.length; i++){
				...
				if(layerId) {
					if(typeof layersbyID[layerId] == 'undefined') layersbyID[layerId] = [];
		...
		hideAnySelectedShapes: function() {
			...
			if (this.map_selected_shape) {
				...
				let targetLayer = null;
				for(let i=0; i < assocLayers.length; i++){
					if( assocLayers[i].get('map_solr_id') == this.map_selected_shape && assocLayers[i].get('map_type') == 'area_map_shape'){
						targetLayer = assocLayers[i]; 
						break;
					}
				} 
				if (targetLayer) {
					console.log('hide the map rectangle', targetLayer, this.map_selected_shape);
		...
		dropPin: function(lon, lat, extent) {
			...
			this.map.getLayers().forEach(layer => {
				...
				if (layer.get('type') && (layer.get('type') == 'current_requested_location' || layer.get('type') == 'current_requested_circle')){
			...
			if (extent?.length) this.map.getView().fit(ol.proj.transformExtent([extent[0], extent[1], extent[2], extent[3]], 'EPSG:4326', this.map.getView().getProjection()), { size: this.map.getSize() });
			...
			if(!this.map_search_shape){				
				...
				var map_circle = new ol.Feature({
					geometry: new ol.geom.Circle(centerLongitudeLatitude, this.map_form.map_overlay_reach * this.mi_to_m_factor),
				})
				map_circle.name = "current_requested_circle";
				map_circle.type = "current_requested_circle";
				map_circle.setStyle(new ol.style.Style({
		...
		filterMap: function() {
			...
			for(var layer of allLayers){
				...
				if(layerType == 'point_map' || layerType == 'area_map_point' || (layerType == "area_map_shape" && layer.getVisible() ) ){
				...
				if (this.map_circle || this.map_search_shape ) {
					...
					if(layerType == 'point_map'){//blue dots
						...
						if(!search_geometry.intersectsCoordinate(layerGeometry.getCoordinates()) ){ 
							should_show = false;				
						} else {
							if(should_show) should_show = true;
						}	

					} else if (layerType == 'area_map_point') {//green dots [and related to green map areas]
						const start = performance.now();
						//check if the point is within the circle extent
						let matchingShape = null;
						for(let i=0; i < assocLayers.length; i++){
							if( assocLayers[i].get('map_solr_id') == layerID && assocLayers[i].get('map_type') == 'area_map_shape'){
								matchingShape = assocLayers[i]; 
								break;
							}
						} 

						const end = performance.now(); exeTimes.push(end - start);

						let layerGeometry = matchingShape.getSource().getFeatures()[0].getGeometry();
						...
						if(matchingShape.getVisible() != should_show && !should_show) matchingShape.setVisible(false);			
					}
				...
				if (should_show !== null && layer.getVisible() != should_show )   layer.setVisible(should_show);
				if (should_show) total_matched++;
			}

			console.timeEnd('Layer Loop Execution Time');
			let sum = exeTimes.reduce( (previousValue, currentValue) => previousValue + currentValue, 0);
			let ave = sum / exeTimes.length;
			console.log(`ave over ${exeTimes.length} dots: `, ave, `-- for a total of ${sum}ms`, exeTimes);
			
			return total_matched;

			if (this.map_circle) {
				let map_overlay_reach = document.querySelector("input[name='map_overlay_reach']")
				console.log('map_overlay_reach:', map_overlay_reach.value);
				console.log("for map", this.map);
				//handle the new circle
				let new_radius = map_overlay_reach.value * this.mi_to_m_factor;
				let search_circle_layer = this.map.getLayers().getArray().find(layer => layer.get('name') == 'current_requested_circle');//get the layer that represents the search circle
				let search_circle_geometry = search_circle_layer.getSource().getFeatures()[0].getGeometry(); 
				...
				let pointLayers = allLayers.filter(layer => { if(!layerTypes.includes(layer.get('map_type'))) layerTypes.push(layer.get('map_type')); return layer.get('map_type') == 'point_map';}); //blue dots
				let mapPointLayers = allLayers.filter(layer => { return layer.get('map_type') == 'area_map_point';}); //green dots
				let mapLayers = allLayers.filter(layer => {return layer.get('map_type') == 'area_map_shape';});
				...
				for(var layer of pointLayers){
					...
					//check if it in the circle (this might be less efficient than more direct ways of comparing the radius to the distance, but more code-clean)
					if(!search_circle_geometry.intersectsCoordinate(layerGeometry.getCoordinates()) ){ 
				...
				for(var layer of mapLayers){
					...
					let layerGeometry = layer.getSource().getFeatures()[0].getGeometry();
					let layerID = layer.get("map_solr_id"); //used to determine the the green pt to use
					let matchingPts = mapPointLayers.filter(l => { return l.get('map_solr_id') == layerID ;});
					let closestPt = layerGeometry.getClosestPoint(search_circle_center); //closest point on the map to the circle center
					let distance = Math.pow(Math.pow(search_circle_center[0] - closestPt[0], 2) + Math.pow(search_circle_center[1] - closestPt[1], 2), .5); //a^2 + b^2 = c^2
		...
		chooseLocationFromBrowser: function() {
			...
			window.navigator.geolocation.getCurrentPosition((evt) => {
		...
		zoomToLocationArea: function() {
			...
			if(layer_to_resize){				
				let extent = layer_to_resize.getSource().getFeatures()[0].getGeometry().getExtent();
				// zoom in, with animation and padding
				this.map.getView().fit(extent, {duration: 1000, size: this.map.getSize(), padding: [50, 50, 50, 300]});
			}
		},
		...
		clickFacet: function(which) {
			console.log('> clickFacet', which, '.facets-'+which.split('_').join('-')+'-swiper');
			if (this.show_facet != which) {
				this.show_facet = which;
				this.$nextTick( () => {
					var facets_swiper = new Swiper('.facets-'+which.split('_').join('-')+'-swiper', {
						slidesPerView: 1,
						breakpoints: {
							500: {slidesPerView: 2},
							768: {slidesPerView: 3},
		...
		scrollTo: function(which_div) {
			...
			var insideRect = document.querySelector('#year-orient-'+which_div).getBoundingClientRect();  // reference to scroll target
			var scrollLocation = Math.abs(insideRect['top'] - outsideRect['top'] + outsideElem.scrollTop) - 30;
			// console.log('>>>>>', outside['top'], inside['top'], scrollLocation, outsideElem.scrollTop);
			document.querySelector('.timeline-years').scrollTo({
				top: scrollLocation, behavior: 'smooth', block: 'end', inline: 'nearest'
			});
		},
