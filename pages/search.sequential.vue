	...
	<div class="advanced-search">
		...
		<div class="search-results">
			<div class="search-sort">
				<h2>{{search_start}} - {{search_end}} of {{search_total}} <button v-if="search_can_load_more" @click="loadMore">Load more {{this.search_page}}</button></h2>
...
export default {
	...
	methods: {
		...
		search: function(load_more=false) {
			console.log('> search', load_more);
			...
			// things for local only search
			let page = '&page=1';
			if (load_more){
				this.remote_search_page++;
				page = '&page=' + (this.remote_search_page + 1);
			}
			...
			if (live_search_requests) {
				...
				let options = {shouldMerge: true, local_filter_requests: local_filter_requests, filter_tag_ids: filter_tag_ids};
		...
		remoteSearch: async function(url, options){
			let defaultOptions = {shouldMerge: false, local_filter_requests: false, filter_tag_ids: []};
			...
			do {
				...
				loopIter++;
				// get results
				 var removeSearchResult = fetch(url)
				.then(res=>res.json())
				.then((response) => {
					...
					if (response.response.pages) {
						...
						if(!local_filter_requests){
							...
							this.search_results = this.remote_search_results;
							...
							if (maps_that_match_tags.length) {
								for (let i=0;i<this.remote_search_results.length;i++) {
									if (maps_that_match_tags.indexOf(this.remote_search_results[i]) !== -1) {
										intersect_results.push(this.remote_search_results[i]);
									}
								}

								console.log('Intersection of internal + live:', intersect_results.length, intersect_results);
						
								this.search_results = intersect_results.slice(0, this.search_perpage * this.search_page);
								this.search_total = intersect_results.length + (this.remote_search_page < this.remote_search_total_pages ? "+" : '')  + ' Results';
								this.search_end = this.search_perpage * this.search_page;
								if (this.search_end > intersect_results.length) this.search_end = intersect_results.length;
								this.search_start = 1;

								//if we need to get more of the maps remotely 
								if (intersect_results.length < this.search_perpage * this.search_page ) {
									...
									if(this.remote_search_page < this.remote_search_total_pages) {
										...
										params.set('page', this.remote_search_page);
		...
		getData: function(which_address){
			...
			if (!which_address) which_address = '';
			...
			.then((response) => {
				if (response.status_type != 'error') {
					// load data into local vars
					console.log('FETCHED!', response);
					// this.tag_groups = response.tag_groups;
					// this.maps_by_years = response.maps_by_years;
					// this.timeline_events = response.timeline_events;
					// this.timeline_eras = response.timeline_eras;
					// this.show_era = Object.keys(response.timeline_eras)[0];// make first one active
					// this.maps = response.maps;
					// // if there's a location lookup
					// this.map_address_lookup_results = response.map_address_lookup_results;
					// do the setup methods
					if (!which_address) {
						// first time here for sure
						this.setupSlider();
						this.clickFacet(Object.keys(this.tag_groups)[0]);// "click" first facet
						this.setupMap();
						// this.setupTimelineScroll();
						...
						// we're looking up by address, so only add that dot (and hide other maps?)
						this.dropPin(this.map_address_lookup_results.lon, this.map_address_lookup_results.lat, this.map_address_lookup_results.extent);
					}
				} else {
					console.log('Looks like there was a problem. Status Code: ' + response.status);
					return;
